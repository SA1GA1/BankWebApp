...
USERS = [
    (77011, "Иван Иванов", "40817810000000000001", "+79001234501", 245_300.50),
    (77002, "Мария Петрова", "40817810000000000002", "+79001234502", 78_100.00),
    (77003, "Алексей Смирнов", "40817810000000000003", "+79001234503", 512_750.20),
    (77004, "Ольга Кузнецова", "40817810000000000004", "+79001234504", 32_500.00),
    (77005, "Дмитрий Соколов", "40817810000000000005", "+79001234505", 154_000.00),
]
...
def seed_if_empty() -> None:
    with get_conn() as conn:
        ...
        # История на главного пользователя (id=77011)
        tx_seed = [
        ...
        for kind, cp, amt, descr, days_ago in tx_seed:
            conn.execute(
                ...
                (77011, kind, cp, amt, descr, _iso(now - timedelta(days=days_ago)), 0.0, "safe"),
            )

        # Диалог 1: безобидный (Иван ↔ Мария)
        thread1 = [
            (77002, 77011, "Привет! Не забудь про встречу завтра в 12 :)", 6),
            (77011, 77002, "Помню, до завтра!", 6),
            (77002, 77011, "Захвати, пожалуйста, кофе по дороге.", 5),
            (77011, 77002, "Окей, какой?", 5),
            (77002, 77011, "Капучино без сахара", 5),
        ]
        ...
        thread2 = [
            (77003, 77011, "Привет, как дела?", 4),
            (77011, 77003, "Привет, всё ок!", 4),
            (
                77003,
                77011,
